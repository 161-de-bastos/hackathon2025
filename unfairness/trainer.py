...
def _build_splits_datasets(train_csv, val_csv, test_csv,
    ...
    ds_tr = ToS(df_tr, tok, max_len)
    ds_va = ToS(df_va, tok, max_len) if df_va is not None else None
    ds_te = ToS(df_te, tok, max_len) if df_te is not None else None
...
def _make_trainer(save_dir, callbacks_config):
    ...
    trainer = pl.Trainer(max_epochs=100, callbacks=[ckpt, es], logger=logger, log_every_n_steps=10, default_root_dir=save_dir)
    return trainer, ckpt
